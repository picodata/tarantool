...
g_two_member_cluster.test_deletion = function(cg)
    ...
    -- On 2.11 there is no box_is_anon()/cfg split (3.x commit 39bec27b9b), so
    -- the recovered instance without an ID fails with ER_UNKNOWN_REPLICA
    -- instead of the "'replication_anon' cfg didn't work" panic.
    local panic_msg = "is not registered with replica set"
