...
## Picodata vendoring notice

This directory is a Picodata-maintained git subtree located at `third_party/metrics`.
It is not a submodule and it is not treated as an upstream-tracking copy of
`tarantool/metrics`.

We are not going to rebase this subtree on upstream. Changes under this
directory are maintained as part of this repository. If an upstream change is
needed, port or cherry-pick the required change explicitly instead of replacing
or rebasing the subtree against upstream history.
...
- [Metrics](#metrics)
	- [Picodata vendoring notice](#picodata-vendoring-notice)
